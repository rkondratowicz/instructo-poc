...
async function validateInstructionContent(instructionPath, useSuperagent = false) {
  try {
    ...
    if (injectionMatches.length > 0) {
      return {
        valid: false,
        errors: [`Potential prompt injection patterns detected: ${injectionMatches.join(', ')}`]
      };
    }

    // Optional Superagent validation if API key is available
    if (useSuperagent && process.env.SUPERAGENT_API_KEY) {
      try {
        const { createClient } = await import('superagent-ai');
        const client = createClient({
          apiKey: process.env.SUPERAGENT_API_KEY
        });

        const guardResult = await client.guard(content);
        if (guardResult.rejected) {
          return {
            valid: false,
            errors: [`Superagent guard blocked: ${guardResult.reasoning}`]
          };
        }
        await client.aclose();
      } catch (error) {
        console.warn(`Superagent validation failed: ${error.message}`);
      }
    }
...
async function validateCatalog() {
    ...
    if (entry.isDirectory()) {
      ...
      if (fs.existsSync(metaPath)) {
        try {
          ...
          if (instructionFile) {
            const fullPath = path.relative(path.join(__dirname, '..'), path.join(dirPath, instructionFile));
            const fullInstructionPath = path.join(dirPath, instructionFile);

            // Validate instruction content for prompt injection
            const contentValidation = await validateInstructionContent(fullInstructionPath, process.argv.includes('--superagent'));
            if (!contentValidation.valid) {
              contentValidation.errors.forEach(error => errors.push(`${entry.name}: ${error}`));
              isValid = false;
            }
  ...
  for (const instruction of catalog.instructions || []) {
    ...
    if (!actual) {
      errors.push(`Instruction "${instruction.name}" in catalog but not found in filesystem`);
      isValid = false;
      continue;
    }
    ...
    if (instruction.path !== actual.path) {
      errors.push(`Path mismatch for "${instruction.name}": expected "${actual.path}", got "${instruction.path}"`);
      isValid = false;
    }
  ...
  for (const [name, actual] of actualInstructions) {
    if (!catalogNames.has(name)) {
      errors.push(`Instruction "${name}" exists in filesystem but missing from catalog`);
      isValid = false;
    }
  }

  // Check if catalog is sorted by name
  const sortedInstructions = [...(catalog.instructions || [])].sort((a, b) => a.name.localeCompare(b.name));
  const isSorted = JSON.stringify(catalog.instructions) === JSON.stringify(sortedInstructions);
  if (!isSorted) {
  ...
  if (isValid) {
    console.log('Validation passed: catalog.json matches the instruction files');
    return true;
  } else {
    console.log('Validation failed:');
    errors.forEach(error => console.log(`- ${error}`));
